...
class TestFamilyTree(unittest.TestCase):
    def setUp(self):
        ...
        self.person3 = Person("person3", "Name3", "LastName3", "1995-11-15", "Place3", "Place3")
    ...
    def test_check_relationship_consistency(self):
        ...
        relationship = Relationship(self.person1.person_id, self.person2.person_id, "spouse")
        self.family_tree.add_person(self.person2, [self.person1])
        self.assertEqual(
    ...
    def test_merge_persons_not_found(self):
        ...
        if isinstance(person2, Person):
            with self.assertRaises(ValueError):
                self.family_tree.merge_persons(self.person1.person_id, person2.person_id)
        

    def test_find_duplicates(self):
        """Test finding duplicate persons."""
        duplicate = Person(
            "person4", "Name1", "LastName1", "1970-01-01", "Place1", "Place1"
        )
        self.family_tree.add_person(duplicate)
        duplicates = self.family_tree.find_duplicates()
        self.assertIn([self.person1, duplicate], [list(duplicate_group) for duplicate_group in duplicates])

    def test_import_export_json(self):
        """Test importing and exporting data in JSON format."""
        relationship = Relationship(self.person1.person_id, self.person3.person_id, "spouse")
        self.family_tree.add_person(self.person3, [self.person1])        
        self.family_tree.link_persons(relationship)
