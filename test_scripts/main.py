...
from sqlalchemy import or_, func, Column, Integer, Boolean, DateTime, Date, ForeignKey, UniqueConstraint
...
# Added missing imports for OpenTelemetry and SQLAlchemy-related types
...
app.config.update({
    ...
    'SESSION_COOKIE_SAMESITE': 'Lax',  # Adjusted for local dev
...
redis_url = os.getenv(
    "REDIS_URL", "redis://localhost:6379/0"  # Default to local Redis
)
...
resource = Resource(
    attributes={
        "service.name": os.getenv(
            "OTEL_SERVICE_NAME", "family-tree-backend"
        )
    }
)
...
otlp_exporter_trace = OTLPSpanExporter(
    # Assuming default endpoint or configured via OTEL_EXPORTER_OTLP_ENDPOINT
)
...
# Reinitialize the tracer after setting the tracer provider
...
ENCRYPTION_KEY_FILE = (
    "/backend/data/encryption_key.json"  # Assuming this path is correct
)
...
def load_encryption_key():
    ...
    key_file_abs_path = os.path.join(
        os.path.dirname(__file__), '..', 'data', 'encryption_key.json'
    )
    logger.info(
        f"Attempting to load encryption key from file: {key_file_abs_path}"
    )
    ...
    try:
        ...
        logger.warning(
            f"Encryption key file not found at {key_file_abs_path}."
        )
        return None
    except json.JSONDecodeError:
        logger.error("Failed to decode encryption key JSON file.")
        return None
    except KeyError:
        ...
        logger.error(
            "Unexpected error loading encryption key from file.",
            exc_info=True
        )
        return None
...
try:
    ...
    if _encryption_key:
        ...
        logger.critical(
            "Encryption key is missing. Fernet cannot be initialized."
        )
        fernet_suite = None
except Exception:
    logger.critical("Failed to init Fernet.", exc_info=True)
    fernet_suite = None
    ...
    def process_bind_param(self, value, dialect):
        if value is not None and fernet_suite:
            try:
                ...
                logger.error("Encryption failed for value.", exc_info=True)
    ...
    def process_result_value(self, value, dialect):
        if value is not None and fernet_suite:
            try:
                ...
                logger.error("Decryption failed for value.", exc_info=True)
    ...
    def to_dict(self, include_sensitive=False):
        ...
        if include_sensitive:
            ...
            data['password_reset_expires'] = (
                self.password_reset_expires.isoformat()
                if self.password_reset_expires else None
            )
        return data
...
class Tree(Base):
    ...
    default_privacy_level = Column(
        SQLAlchemyEnum(PrivacyLevelEnum),
...
class TreeAccess(Base):
    ...
    access_level = Column(
        String(50),
        nullable=False,
        default="view"  # Consider an Enum here too
    )
    granted_by = Column(UUID(as_uuid=True), ForeignKey("users.id"))
    granted_at = Column(DateTime, default=datetime.utcnow)
    __table_args__ = (
        UniqueConstraint(
            "tree_id", "user_id", name="tree_user_unique"
        ),
    )
...
class Person(Base):
    ...
    privacy_level = Column(
        SQLAlchemyEnum(PrivacyLevelEnum),
        default=PrivacyLevelEnum.inherit
    )
...
class ActivityLog(Base):
    ...
    )  # Optional tree link, SET NULL on tree delete
    user_id = Column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL")
    )  # Optional user link, SET NULL on user delete
...
def _handle_sqlalchemy_error(e: SQLAlchemyError, context: str, db: Session):
    ...
    if isinstance(e, IntegrityError):
        ...
        if constraint_name == 'users_username_key':
            logger.warning(
                f"Integrity Error: Duplicate username during {context}",
                exc_info=False
            )
            abort(409, description="Username already exists.")
        elif constraint_name == 'users_email_key':
            logger.warning(
                f"Integrity Error: Duplicate email during {context}",
                exc_info=False
            )
            abort(409, description="Email already exists.")
        elif constraint_name == 'tree_user_unique':
            logger.warning(
                f"Integrity Error: Duplicate tree access entry during {context}",
                exc_info=False
            )
            abort(409, description="User already has access to this tree.")
        elif 'foreign key constraint' in str(e.orig).lower():
            logger.warning(
                f"Integrity Error: Foreign key violation during {context}",
                exc_info=False
            )
            ...
            logger.warning(
                f"Integrity Error: Not null constraint failed during {context}",
                exc_info=False
            )
            abort(400, description=f"Missing required field during {context}.")
        else:
            logger.error(
                f"Unhandled Integrity Error during {context}",
                exc_info=True
            )
        ...
        logger.error(
            f"Unhandled SQLAlchemy Error during {context}",
            exc_info=True
        )
...
def _get_or_404(
    ...
    with tracer.start_as_current_span(f"db.get.{model.__name__}") as span:
        ...
        try:
            ...
            db_operation_duration_histogram.record(
                duration,
                {
                    "db.operation": f"get.{model.__name__}",
                    "db.status": "success"
                }
            )
            ...
            _handle_sqlalchemy_error(
                e,
                f"fetching {model.__name__} ID {model_id}",
                db
            )
        except Exception:
            logger.error(
                f"Unexpected error fetching {model.__name__} ID {model_id}",
                exc_info=True
            )
...
def require_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:
            logger.warning(
                "Authentication required, session not found.",
                session_contents=session
            )
...
def require_admin(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session or session.get('role') != UserRole.ADMIN.value:
            logger.warning(
                "Tree access required, but user not authenticated."
            )
...
def require_tree_access(level: str = 'view'):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not tree_id_str:
                logger.warning(
                    "Tree access required, but no active tree set in session.",
                    user_id=user_id
                )
            ...
            try:
                ...
                logger.warning(
                    "Tree access required, but active_tree_id in session is invalid UUID.",
                    user_id=user_id,
            ...
            if not tree:
                logger.warning(
                    "Tree access check failed: Tree not found.",
                    user_id=user_id,
                    tree_id=tree_id
                )
...
def get_activity_log_db(
    ...
    with tracer.start_as_current_span("service.get_activity_log") as span:
        ...
        try:
            ...
            logger.error(
                "Database error fetching activity logs.", exc_info=True
            )
            _handle_sqlalchemy_error(e, "fetching activity logs", db)
        except Exception:
            logger.error(
                "Unexpected error fetching activity logs.", exc_info=True
            )
            abort(
                500,
                description="An unexpected error occurred while fetching activity logs."
            )
...
def delete_tree_db(db: Session, tree_id: uuid.UUID) -> None:
    ...
    with tracer.start_as_current_span("service.delete_tree") as span:
        ...
        try:
            ...
            db.rollback()
            logger.error(
                "Database error during tree deletion.",
                tree_id=tree_id,
                exc_info=True
            )
            ...
            logger.error(
                "Unexpected error during tree deletion.",
                tree_id=tree_id,
                exc_info=True
            )
...
def update_tree_db(
    ...
    with tracer.start_as_current_span("service.update_tree") as span:
        ...
        try:
            ...
            db.rollback()
            logger.error(
                "Database error during tree update.",
                tree_id=tree_id,
                exc_info=True
            )
            ...
            logger.error(
                "Unexpected error during tree update.",
                tree_id=tree_id,
                exc_info=True
            )
...
def update_user_role_db(
    ...
    with tracer.start_as_current_span("service.update_user_role") as span:
        ...
        try:
            ...
            logger.error(
                "Unexpected error during user role update.",
                user_id=user_id,
                exc_info=True
            )
...
def register_user_db(db: Session, user_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with tracer.start_as_current_span("service.register_user") as span:
        ...
        try:
            ...
            logger.warning(
                "User registration failed: Password complexity requirements not met.",
        ...
        try:
            ...
            db.rollback()
            logger.warning(
                "User registration failed: Integrity error.",
                username=user_data['username'],
                exc_info=True
            )
...
def request_password_reset_db(db: Session, email_or_username: str) -> bool:
    ...
    with tracer.start_as_current_span("service.request_password_reset") as span:
        ...
        if not fernet_suite:
            logger.error(
                "Password reset failed: Encryption suite not initialized."
            )
        ...
        try:
            ...
            if email_user and email_password and email_server and email_port:
                try:
                    ...
                    logger.info(
                        "Password reset email sent (simulated/placeholder).",
                        user_id=user.id,
                        email=user.email
                    )
                ...
                logger.warning(
                    "Email sending is not configured. Password reset link generated but not sent.",
                    user_id=user.id,
            ...
            logger.info(
                "Password reset token generated and saved.",
                user_id=user.id,
    ...
    with tracer.start_as_current_span("service.reset_password") as span:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Password reset failed: New password complexity requirements not met.",
                    user_id=user.id,
                    reason=str(e)
                )
                abort(400, description=str(e))
    ...
    with tracer.start_as_current_span("service.create_tree") as span:
        ...
        try:
            new_tree = Tree(
                ...
                default_privacy_level=PrivacyLevelEnum(
                    tree_data.get(
                        'default_privacy_level',
                        PrivacyLevelEnum.private.value
                    )
                ),
                created_at=datetime.utcnow(),
            ...
            db.rollback()
            logger.error(
                "Database error during tree creation.",
                user_id=user_id,
                tree_name=tree_name,
                exc_info=True
            )
            ...
            db.rollback()
            logger.warning(
                "Invalid privacy level during tree creation.",
                user_id=user_id,
                tree_name=tree_name,
                error=str(e)
            )
            ...
            logger.error(
                "Unexpected error during tree creation.",
                user_id=user_id,
                tree_name=tree_name,
                exc_info=True
            )
...
def get_user_trees_db(db: Session, user_id: uuid.UUID) -> List[Dict[str, Any]]:
    ...
    with tracer.start_as_current_span("service.get_user_trees") as span:
        ...
        try:
            ...
            trees = db.query(Tree).join(
                TreeAccess,
                Tree.id == TreeAccess.tree_id
            ).filter(
            ...
            logger.error(
                "Database error fetching user trees.",
                user_id=user_id,
                exc_info=True
            )
            span.record_exception(e)
            ...
            logger.error(
                "Unexpected error fetching user trees.",
                user_id=user_id,
                exc_info=True
            )
...
def get_all_people_db(db: Session, tree_id: uuid.UUID) -> List[Dict[str, Any]]:
    ...
    with tracer.start_as_current_span("service.get_all_people") as span:
        ...
        try:
            ...
            logger.error(
                "Unexpected error fetching all people for tree.",
                tree_id=tree_id,
                exc_info=True
            )
    ...
    with tracer.start_as_current_span("service.create_person") as span:
        ...
        if birth_date_str:
            try:
                ...
                logger.warning(
                    "Person creation failed: Invalid birth date format.",
                ...
                abort(
                    ...
                    description={
                        "details": {
                            "birth_date": "Invalid date format (YYYY-MM-DD)."
                        }
                    }
                )
        ...
        if death_date_str:
            try:
                ...
                logger.warning(
                    "Person creation failed: Invalid death date format.",
                ...
                abort(
                    ...
                    description={
                        "details": {
                            "death_date": "Invalid date format (YYYY-MM-DD)."
                        }
                    }
                )
        ...
        if birth_date and death_date and death_date < birth_date:
            logger.warning(
                "Person creation failed: Death date before birth date."
            )
            abort(
                400,
                description={
                    "details": {
                        "date_comparison": "Death date cannot be before birth date."
                    }
                }
            )
        ...
        try:
            new_person = Person(
                ...
                privacy_level=PrivacyLevelEnum(
                    person_data.get(
                        'privacy_level',
                        PrivacyLevelEnum.inherit.value
                    )
                ),
                is_living=person_data.get(
                    'is_living'  # Needs logic to determine if not provided
                ),
            ...
            db.rollback()
            logger.error(
                "Database error during person creation.",
                user_id=user_id,
                tree_id=tree_id,
                person_name=person_name,
                exc_info=True
            ...
            db.rollback()
            logger.warning(
                "Invalid privacy level during person creation.",
                user_id=user_id,
                tree_id=tree_id,
                person_name=person_name,
                error=str(e)
            ...
            logger.error(
                "Unexpected error during person creation.",
                user_id=user_id,
                tree_id=tree_id,
                person_name=person_name,
                exc_info=True
            ...
            abort(
                500,
                description="An unexpected error occurred during person creation."
            )
