...
# --- Custom Exceptions ---
class PersonLimitReachedException(Exception):
    """Exception raised when the maximum number of people has been created."""
    pass
...
class APIClient:
    ...
    def create_person(self, first_name, last_name, birth_date, gender, death_date=None, notes=""):
        ...
        if MAX_PERSONS_LIMIT is not None and PERSON_CREATION_COUNT >= MAX_PERSONS_LIMIT:
            ...
            # Raise custom exception to signal that we need to shutdown
            raise PersonLimitReachedException(
                f"Maximum person limit of {MAX_PERSONS_LIMIT} reached. Shutting down gracefully."
            )
...
def create_family_tree_for_user(api_client, config, user_num, tree_num_for_user):
    ...
    except PersonLimitReachedException as e:
        logger.info(f"Person limit reached while creating tree '{tree_name}'. {str(e)}")
...
def main():
    ...
    try:
        ...
        logger.info("Demo data loading process finished successfully.")
    except PersonLimitReachedException as e:
        logger.info(f"Demo data loading process completed early: {str(e)}")
        logger.info(f"Total persons created: {PERSON_CREATION_COUNT}")
        # Exit gracefully - we've reached our limit as expected
        return
    except Exception as e:
        logger.error(f"An unexpected error occurred during demo data loading: {str(e)}")
        # For debugging, you might want to include the full stack trace
        import traceback
