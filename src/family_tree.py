...
class FamilyTree:
    # --- Keep __init__ and other methods as defined in family_tree_py_init_fix ---
    ...
    def get_people_summary(self):
        ...
        for p in self.people.values():
             ...
             summary_list.append({
                 "person_id": p.person_id, "name": full_name, "display_name": display_name,
    ...
    def get_relationships_summary(self):
        ...
        for rel_id, rel in self.relationships.items():
            ...
            summary.append({
                ...
                "relationship_type": rel.rel_type, "_person1_sort_name": person1_sort_name
            })
    ...
    def get_nodes_links_data(self):
        """
        Generates data in nodes and links format.
        Links are oriented parent -> child for hierarchy building.
        Other relationship types are included for potential separate rendering.
        ...
        nodes = []
        links = []
        person_ids = list(self.people.keys()) # Get a fixed list of IDs
        ...
        # Create links, orienting parent/child relationships correctly
        processed_rel_ids = set() # Avoid processing reciprocal relationships twice if stored both ways
        for rel_id, rel in self.relationships.items():
            ...
            rel_type = rel.rel_type.lower() # Use lowercase type

            # Ensure both source and target persons exist before creating link
            if source_id not in self.people or target_id not in self.people:
                 print(f"Warning: Skipping relationship {rel_id} because person {source_id} or {target_id} not found.")
                 continue

            link_data = None
            if rel_type == 'parent':
                # Relationship is Parent(P1) -> Child(P2)
                link_data = {"source": source_id, "target": target_id, "type": "parent_child"}
            elif rel_type == 'child':
                # Relationship is Child(P1) -> Parent(P2)
                # We want Parent -> Child for hierarchy, so reverse source/target
                link_data = {"source": target_id, "target": source_id, "type": "parent_child"}
            elif rel_type == 'spouse' or rel_type == 'partner':
                 # Keep spouse/partner links as is for now
                 link_data = {"source": source_id, "target": target_id, "type": rel_type}
                 # Mark the reciprocal as processed if it exists (e.g., p2->p1 spouse)
                 # This simple check assumes reciprocal rels share attributes/type exactly
                 reciprocal_rel = next((r for r_id, r in self.relationships.items()
                                       if r.person1_id == target_id and r.person2_id == source_id and r.rel_type.lower() == rel_type), None)
                 if reciprocal_rel:
                     processed_rel_ids.add(reciprocal_rel) # Need the ID of the reciprocal rel
            else:
    ...
    # --- Keep remaining methods (_to_dict, _from_dict, save_tree, load_tree, edit_person, delete_person) ---
    # ... (rest of the methods from family_tree_py_init_fix) ...
    def _to_dict(self):
        return {
    ...
    def _from_dict(cls, data, tree_file_path, audit_log_path):
        tree = cls(tree_file_path, audit_log_path)
        tree.people = {}
        ...
        try:
            ...
            if data:
                ...
                print(f"Family tree loaded successfully from {self.tree_file_path}. Found {len(self.people)} people and {len(self.relationships)} relationships.")
                 ...
                 self.people = {}; self.relationships = {}
        ...
        if person:
            ...
            if changes_made:
                ...
                self.save_tree(edited_by); return True
                 ...
                 log_audit(self.audit_log_path, edited_by, 'edit_person', f'no changes made for id: {person_id}'); return False
        else:
    ...
    def delete_person(self, person_id, deleted_by="system"):
        ...
        if person:
            ...
            log_audit(self.audit_log_path, deleted_by, 'delete_person', f'failure - person not found: {person_id}'); return False
