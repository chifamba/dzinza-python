...
class UserManagement:
    ...
    def create_user(self, username: str, password: str, role: str = "basic") -> Optional[User]:
        ...
        try:
            ...
            new_user = User(
                username=username,
                password_hash_b64=hashed_password_b64.decode('utf-8'), # Store as string in DB model? Check model definition
                role=role
            )
    ...
    def login_user(self, username: str, password: str) -> Optional[User]:
        ...
        try:
            ...
            # Ensure password_hash_b64 is treated as bytes for verification
            if user and self._verify_password(user.password_hash_b64.encode('utf-8'), password):
    ...
    def change_password(self, username: str, old_password: str, new_password: str) -> bool:
        ...
        try:
            user = self.db.query(User).filter(User.username == username).first()
            if user and self._verify_password(user.password_hash_b64.encode('utf-8'), old_password):
                user.password_hash_b64 = self._hash_password(new_password).decode('utf-8') # Store as string
                self.db.commit()
