# backend/services/user_service.py
import uuid
import structlog
from datetime import datetime, timedelta
import secrets # For password reset token
from sqlalchemy.orm import Session as DBSession
from sqlalchemy.exc import SQLAlchemyError, IntegrityError
from sqlalchemy import or_
from flask import abort, current_app # For config access, abort

from ..models import User, UserRole
from ..utils import (_validate_password_complexity, _hash_password, _verify_password,
                     _get_or_404, _handle_sqlalchemy_error, paginate_query)
from ..config import PAGINATION_DEFAULTS, DEFAULT_PAGE, DEFAULT_PAGE_SIZE, config as app_config # Import app_config
from ..extensions import user_registration_counter, auth_failure_counter, role_change_counter, get_fernet
# from opentelemetry import trace

# tracer = trace.get_tracer(__name__)
logger = structlog.get_logger(__name__)


def register_user_db(db: DBSession, user_data: Dict[str, Any]) -> Dict[str, Any]:
    """Registers a new user."""
    username = user_data.get('username')
    email = user_data.get('email')
    password = user_data.get('password')
    logger.info("Registering new user", username=username, email=email)

    if not all([username, email, password]):
        abort(400, description="Username, email, and password are required.")

    try:
        _validate_password_complexity(password)
    except ValueError as e:
        abort(400, description=str(e))
    
    hashed_password = _hash_password(password)
    
    try:
        role_value = user_data.get('role', UserRole.USER.value)
        user_role_enum = UserRole(role_value)
    except ValueError:
        abort(400, description=f"Invalid role: {role_value}. Valid: {[r.value for r in UserRole]}.")

    try:
        new_user = User(
            username=username, email=email.lower(), password_hash=hashed_password,
            full_name=user_data.get('full_name'), role=user_role_enum,
            is_active=True, email_verified=False # Default activation, email not verified
        )
        db.add(new_user)
        db.commit()
        db.refresh(new_user)
        if user_registration_counter: user_registration_counter.add(1, {"status": "success"})
        logger.info("User registered successfully", user_id=new_user.id, username=new_user.username)
        return new_user.to_dict()
    except IntegrityError as e:
        if user_registration_counter: user_registration_counter.add(1, {"status": "failure", "reason": "integrity_error"})
        _handle_sqlalchemy_error(e, "registering user", db) # Handles specific duplicate messages
    except SQLAlchemyError as e:
        if user_registration_counter: user_registration_counter.add(1, {"status": "failure", "reason": "db_error"})
        _handle_sqlalchemy_error(e, "registering user", db)
    except Exception as e:
        db.rollback()
        if user_registration_counter: user_registration_counter.add(1, {"status": "failure", "reason": "unknown_error"})
        logger.error("Unexpected error during user registration.", username=username, exc_info=True)
        abort(500, description="An unexpected error occurred during registration.")
    return {} # Should be unreachable

def authenticate_user_db(db: DBSession, username_or_email: str, password: str) -> Optional[Dict[str, Any]]:
    """Authenticates a user by username/email and password."""
    logger.info("Authenticating user", identifier=username_or_email)
    if not username_or_email or not password:
        return None # Or abort(400)

    try:
        normalized_identifier = username_or_email.lower()
        user = db.query(User).filter(
            or_(User.username == username_or_email, User.email == normalized_identifier)
        ).one_or_none()

        if not user:
            if auth_failure_counter: auth_failure_counter.add(1, {"reason": "user_not_found"})
            return None
        if not user.is_active:
            if auth_failure_counter: auth_failure_counter.add(1, {"reason": "inactive_account", "user_id": str(user.id)})
            abort(401, description="Account is inactive.")
        if not _verify_password(password, user.password_hash):
            if auth_failure_counter: auth_failure_counter.add(1, {"reason": "incorrect_password", "user_id": str(user.id)})
            return None
        
        user.last_login = datetime.utcnow()
        db.commit()
        db.refresh(user)
        logger.info("Authentication successful", user_id=user.id, username=user.username)
        return user.to_dict(include_sensitive=False)
    except SQLAlchemyError as e:
        _handle_sqlalchemy_error(e, "authenticating user", db)
    except HTTPException: # Re-raise aborts (like from inactive account)
        raise
    except Exception as e:
        logger.error("Unexpected error during user authentication.", identifier=username_or_email, exc_info=True)
        abort(500, "An unexpected error occurred during authentication.")
    return None

def get_all_users_db(db: DBSession,
                     page: int = DEFAULT_PAGE,
                     per_page: int = DEFAULT_PAGE_SIZE,
                     sort_by: Optional[str] = "username",
                     sort_order: Optional[str] = "asc"
                     ) -> Dict[str, Any]:
    """Fetches a paginated list of all users (typically for admin)."""
    logger.info("Fetching all users from DB", page=page, per_page=per_page)
    try:
        query = db.query(User)
        if not hasattr(User, sort_by or ""):
            logger.warning(f"Invalid sort_by column '{sort_by}' for User. Defaulting to 'username'.")
            sort_by = "username"
        return paginate_query(query, User, page, per_page, PAGINATION_DEFAULTS["max_per_page"], sort_by, sort_order)
    except SQLAlchemyError as e:
        _handle_sqlalchemy_error(e, "fetching all users", db)
    except Exception as e:
        logger.error("Unexpected error fetching all users", exc_info=True)
        abort(500, "An unexpected error occurred while fetching users.")
    return {} # Should be unreachable

def request_password_reset_db(db: DBSession, email_or_username: str) -> bool:
    """Generates a password reset token and (simulates) sending an email."""
    logger.info("Password reset request received", identifier=email_or_username)
    
    fernet = get_fernet() # Get Fernet instance from extensions
    if not fernet: # Should be initialized by app factory
         logger.error("Password reset failed: Encryption suite (Fernet) not initialized.")
         abort(503, description="Password reset service is temporarily unavailable.")

    normalized_identifier = email_or_username.lower()
    user = db.query(User).filter(
        or_(User.username == email_or_username, User.email == normalized_identifier)
    ).one_or_none()

    if not user or not user.is_active: # Do not reveal if user exists or is inactive
        logger.warning("Password reset request for non-existent or inactive user.", identifier_prefix=email_or_username[:5])
        return True # Pretend success

    try:
        raw_token = secrets.token_urlsafe(32)
        user.password_reset_token = raw_token # Store raw for simplicity, or hash it
        user.password_reset_expires = datetime.utcnow() + timedelta(hours=1)
        db.commit()

        reset_link = f"{app_config.FRONTEND_APP_URL}/reset-password/{raw_token}"
        email_configured = all([app_config.EMAIL_USERNAME, app_config.EMAIL_PASSWORD, app_config.EMAIL_SERVER, app_config.EMAIL_PORT])

        if email_configured:
             logger.info(f"Simulating password reset email to {user.email} with link: {reset_link}", user_id=user.id)
             # Actual email sending logic would go here
        else:
            logger.warning("Email sending not configured. Reset link (for log):", reset_link=reset_link, user_id=user.id)
        
        logger.info("Password reset token generated.", user_id=user.id)
        return True
    except SQLAlchemyError as e:
        _handle_sqlalchemy_error(e, "requesting password reset", db)
    except Exception as e:
        db.rollback()
        logger.error("Unexpected error during password reset request.", user_id=getattr(user, 'id', None), exc_info=True)
        abort(500, description="An unexpected error occurred during password reset.")
    return False

def reset_password_db(db: DBSession, token: str, new_password: str) -> bool:
    """Resets a user's password using a valid token."""
    logger.info("Attempting to reset password with token", token_prefix=token[:8])
    if not token or not new_password:
        abort(400, description="Token and new password are required.")

    try:
        user = db.query(User).filter(
            User.password_reset_token == token,
            User.password_reset_expires > datetime.utcnow()
        ).one_or_none()

        if not user:
            abort(400, description="Invalid or expired password reset token.")
        
        _validate_password_complexity(new_password)
        
        user.password_hash = _hash_password(new_password)
        user.password_reset_token = None
        user.password_reset_expires = None
        user.email_verified = True # Optionally verify email
        db.commit()
        logger.info("Password reset successful.", user_id=user.id)
        return True
    except ValueError as ve: # From password complexity validation
        abort(400, description=str(ve))
    except SQLAlchemyError as e:
        _handle_sqlalchemy_error(e, "resetting password", db)
    except Exception as e:
        db.rollback()
        logger.error("Unexpected error during password reset.", exc_info=True)
        abort(500, description="An unexpected error occurred during password reset.")
    return False

def update_user_role_db(db: DBSession, user_id: uuid.UUID, new_role_str: str) -> Dict[str, Any]:
    """Updates a user's role (admin operation)."""
    logger.info("Updating user role", target_user_id=user_id, new_role=new_role_str)
    try:
        new_role_enum = UserRole(new_role_str)
    except ValueError:
        abort(400, description=f"Invalid role: {new_role_str}. Valid: {[r.value for r in UserRole]}.")

    user = _get_or_404(db, User, user_id)
    if user.role == new_role_enum:
        logger.info("User role is already set to target role.", user_id=user.id)
        return user.to_dict()

    user.role = new_role_enum
    try:
        db.commit()
        db.refresh(user)
        if role_change_counter: role_change_counter.add(1, {"target_user_id": str(user_id), "new_role": new_role_str})
        logger.info("User role updated successfully", user_id=user.id, new_role=user.role.value)
        return user.to_dict()
    except SQLAlchemyError as e:
        _handle_sqlalchemy_error(e, "updating user role", db)
    except Exception as e:
        db.rollback()
        logger.error("Unexpected error during user role update.", user_id=user_id, exc_info=True)
        abort(500, description="An unexpected error occurred while updating user role.")
    return {} # Should be unreachable

def delete_user_db(db: DBSession, user_id: uuid.UUID) -> None:
    """Deletes a user (admin operation)."""
    logger.info("Deleting user", user_id=user_id)
    user = _get_or_404(db, User, user_id)
    try:
        db.delete(user)
        db.commit()
        logger.info("User deleted successfully", user_id=user_id)
    except IntegrityError as ie: # If user owns trees, etc.
        db.rollback()
        if "violates foreign key constraint" in str(ie.orig).lower():
            abort(409, description="Cannot delete user: user owns data (e.g., trees). Reassign or delete their data first.")
        _handle_sqlalchemy_error(ie, "deleting user (integrity)", db)
    except SQLAlchemyError as e:
        _handle_sqlalchemy_error(e, "deleting user", db)
    except Exception as e:
        db.rollback()
        logger.error("Unexpected error during user deletion.", user_id=user_id, exc_info=True)
        abort(500, description="An unexpected error occurred while deleting the user.")

